...
app.use((req, res) => {
  ...
  // Disable Server Side rendering flag
  if (__DISABLE_SSR__) {
    hydrateOnClient()
    return
  }
  
  // from react-router API
  // 
  match({ history, routes: getRoutes(), location: req.url }, (error, redirectLocation, renderProps) => {
    if (error) {
      ...
      loadOnServer({ ...renderProps, store }).then(() => { // from middleware redux-connect - delay the execution of our component
